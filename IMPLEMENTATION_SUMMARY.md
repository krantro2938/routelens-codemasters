...
> [README и команды запуска](README.md) · [Финальный Observatory](routing_observatory_test.html) · [Финальные решения](routing_decisions_test.json) · [Финальный отчёт](routing_report_test.json) · [Публичный Observatory](routing_observatory.html)

Первые два раздела дают быстрый маршрут для судьи; остальная часть служит адресной картой доказательств по критериям. README содержит команды воспроизведения, а здесь собраны архитектура, соответствие заданию и ключевые файлы.
...
- автоматизированные тесты: **155 тестов, 843 проверки, 0 сбоев**;
...
| `routing_decisions_test.json`, `routing_report_test.json` | Финальные обязательные файлы для 90 операций тестовой очереди |
| `routing_observatory_test.html` | Автономное представление финальных решений и отчёта для жюри |
...
- Внешний вектор `traffic_percentage` обязан суммироваться до 100% с допуском на округление; отдельно допустимые 0–100 значения не могут образовать модель на 300%.
...
Воспроизводимый выпуск финальной очереди `operations_queue_test.json`:

```bash
bin/release_test_case
```

Команда всегда использует официальные пути и создаёт `routing_decisions_test.json`, `routing_report_test.json` и `routing_observatory_test.html`. Затем `scripts/release_check.rb` проверяет обязательную пару JSON:
...
| Тесты | 155 тестов, 843 проверки, всё пройдено |
| Публичный валидатор | 29 пройдено, 0 ошибок, 0 предупреждений |
...
routing_observatory_test.html    Автономная панель финальной очереди
```
...
Финальная очередь `operations_queue_test.json` обработана полностью: в корне находятся 90 решений, итоговый отчёт и автономный Observatory. Защищённая команда воспроизводит артефакты и семантически проверяет обязательную пару JSON.
